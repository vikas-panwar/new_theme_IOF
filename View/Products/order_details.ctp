...
<script>
    ...
    function disableTipOption(isDisable) {
         if(isDisable) {
            var selectedTip = $('#OrderTip').val();
            switch (selectedTip) {
                  case null : $("#OrderTip").val("0").trigger('change'); break;
                  case "2" : $("#OrderTip").val("1").trigger('change'); break;//Tip With Card
                  case "3" : $("#OrderTip").val("1").trigger('change'); break; //Tip %
                  default : break;
            }
            $('#OrderTip option[value="2"]').attr('disabled',true);
            $('#OrderTip option[value="3"]').attr('disabled',true);
        } else {
            $('#OrderTip option[value="2"]').attr('disabled',false);
            $('#OrderTip option[value="3"]').attr('disabled',false);
        }
    }
</script>
