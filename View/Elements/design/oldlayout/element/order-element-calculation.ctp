...
<style>
    ...
    .iodr-table .offerItems a b { font-size:14px;color:rgb(136, 108, 88);cursor:pointer;position:absolute;top:0;right:0;}
            ...
            <table style="width:100%;" cellpadding="0" cellspacing="0">
                ...
                if (!empty($itemDisplayArray['item'])) {
                    foreach ($itemDisplayArray['item'] as $keyIndex => $orderDetail) {
                        echo '<tr><td colspan="3"><strong class="common-bold common-bold-cat">' . $orderDetail['category_name'] . '</strong></td></tr>';
                        echo '<tr><td>' . $orderDetail['item_quantity'] . '</td><td>' . $orderDetail['item_size'] . ' ' . $orderDetail['item_name'] . '</td><td>' . ($orderDetail['item_total_price_with_quantity']) . '<strong>' . $orderDetail['tax_label'] . '</strong>' . '<item class="itemRemove" id=' . "$keyIndex" . ' style="float:right;"><a href="javascript:void(0)" title="Remove Item"><i class="fa fa-times"></i></a></item></td></tr>';
