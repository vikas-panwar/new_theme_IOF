...
<ul class="order-overview-listing clearfix">
    <li class='dateTimeInfo'>
        <span class="title"><label><span class="common-bold datelabel"><?php echo ($ordertype == 3) ? 'Delivery' : 'Pick Up'; ?> Date</span><br>
            </label></span>
        <div class="title-box dateInfo">
            <?php echo $this->Session->read('Order.store_pickup_date'); ?>           
        </div>

        <span class="title"><label><span class="common-bold timelabel"><?php echo ($ordertype == 3) ? 'Delivery' : 'Pick Up'; ?> Time</span><br>
            </label></span>
...
//                            $paidpreference .= $subdetails['SubPreference']['name'] . " ,";
    ...
    if ($this->Session->check('Zone.fee') && $ordertype == 3) {
        ...
        <li>
            ...
            <div class="title-box">$<?php
                ...
                if ($this->Session->check('Zone.fee')) {
                    echo number_format($this->Session->read('Zone.fee'), 2);
                } else {
                    //echo number_format($this->Session->read('delivery_fee'), 2);
                }
                ?></div>
    ...
    if (isset($_SESSION['tip']) && ($_SESSION['tip'] > 0)) {
        ...
        ?>
        <li>
            <span class="title"><label>Add Tip</label></span>
    ...
    <li>
        ...
        ?>
        <span class="title tipsection"><label class="common-bold common-size">Total</label></span>
    ...
    if (AuthComponent::User()) {
        if (!empty($storeSetting['StoreSetting']['save_to_order_btn'])) {
            ?>
            <li id="desktop_save"><?php echo $this->Form->button('SAVE TO ORDERS', array('type' => 'button', 'onclick' => "window.location.href='/payments/paymentSection'", 'class' => 'btn green-btn', 'style' => 'float:right;')); ?>
            </li>

            <li id="mobile_save">
                <?php echo $this->Form->button('SAVE FOR LATER', array('type' => 'button', 'onclick' => "window.location.href='/payments/paymentSection'", 'class' => 'btn green-btn', 'style' => 'float:right;')); ?>
            </li>
            <?php
        }
    }
