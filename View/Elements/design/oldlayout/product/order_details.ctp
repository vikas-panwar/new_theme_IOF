    ...
    <section class="form-layout pickup-form pickup-order-form">
        ...
        <div class="check-btn space20 delivery-address-option">
            <?php if ($store_result['Store']['is_delivery'] == 1) { ?>
                <input type="radio" class="ordertype changeOrderType" id="order_type_1"  value="3" name="orderType"  <?php echo ($ordeType == 3) ? "checked" : ''; ?> />
                <label for="order_type_1" class="common-bold common-size">
                    <span></span>Delivery
                </label> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <?php } ?>
            <?php if ($store_result['Store']['is_take_away'] == 1) { ?>	
                <input type="radio" class="ordertype changeOrderType" id="order_type_2"  value="2" name="orderType" <?php echo ($ordeType == 2) ? "checked" : ''; ?> />
        ...
        if ($setPre == 1) {
            ...
            if ($PreorderAllowed) {
                ?>
                <div class="radio-btn space20 delivery-address-option radio-btn-margin-top">
                    <input type="radio" class="changeOrderType" id="pre-order" name="deliveryType" checked value="1" />
                    <label for="pre-order" class="common-bold common-size">
                        <span></span>Pre Order
                    </label>
                </div>
                <?php
            ...
            <div class="radio-btn space20 delivery-address-option radio-btn-margin-top">
                <?php if ($nowAvail) { ?>
                    <input type="radio" class="changeOrderType" id="now" name="deliveryType" value="0" <?php echo ($preOrder == 0) ? "checked" : ''; ?> />
                    <label for="now" class="common-bold common-size hideNow">
                        <span></span>Now
                    </label>
                <?php } ?>
                <?php if ($PreorderAllowed) { ?>
                    <input type="radio" class="changeOrderType" id="pre-order" name="deliveryType" value="1" <?php echo ($preOrder == 1) ? "checked" : ''; ?> />
                    <label for="pre-order" class="common-bold common-size">
                        <span></span>Pre Order
                    </label>
                <?php } ?>
        ...
        if ($setPre != 1 || $PreorderAllowed == 1) {
            ...
            <ul id="pickupdata" class="clearfix">
                ...
                <!--	    <li>
                                <span class="title"><label><span  class='timelabel'>Delivery Time</span><em>*</em></label></span>
                                <div class="title-box"><span id="resvTime">
                <?php
                if (empty($time_range)) {
                    
                    echo $this->Form->input('Store.pickup_time', array('type' => 'select', 'class' => 'inbox', 'empty' => 'Store is closed on this day', 'options' => $time_range, 'label' => false, 'div' => false));
                } else {
                    ?>
                                                                                                                                                                                                                <select id="StorePickupTime" class="inbox" name="data[Store][pickup_time]">
                    ...
                    foreach ($time_range as $key => $value) {
                        ...
                        foreach ($storeBreak as $breakKey => $breakVlue) {
                            if (strtotime($storeBreak[$breakKey]['start']) <= strtotime($key) && strtotime($storeBreak[$breakKey]['end']) >= strtotime($key)) {
                                if ($deliveryTime == $key) {
                                    echo "<option value='$key' selected disabled='disabled'>$value - Break Time </option>";
                                } else {
                ...
                <?php echo $this->Form->error('Store.pickup_time'); ?></div>
                            </li>-->
        ...
        <?php } else { ?>
            ...
            <?php if ($nowAvail) { ?>
                <input type="radio" class="changeOrderType" id="now" name="deliveryType" value="0" <?php echo ($preOrder == 0) ? "checked" : ''; ?> />
                <label for="now" class="common-bold common-size">
                    <span></span>Now
                <?php } ?>
            </label><div class="button-frame text-right">
                <button type="button" class="btn green-btn changeOrderType" id="changeOrderType">CHANGE</button>
            </div>
