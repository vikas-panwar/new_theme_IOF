...
if (!empty($graphData['Store']) && !empty($stores)) 
{
    ...
    $aData = $allTotalItem = $oData = array();
    if($graphPageNumber == 0)
    {
            ...
            if (!empty($dataStore)) 
            {
                foreach ($dataStore as $akey => $itemArray) 
                {
                    if (isset($itemArray) && !empty($itemArray)) 
                    {
                        ...
                        if(array_key_exists('totalitems', $itemArray) && isset($itemArray['totalitems']))
                        {
                            if(isset($aData[$akey]['totalitems']))
                            {
                                $aData[$akey]['week'] = (isset($aData[$akey]['week']) ? $aData[$akey]['week'] : $akey);
                                $aData[$akey]['totalitems'] = $aData[$akey]['totalitems'] + $itemArray['totalitems'];
                                $totalItems                += $itemArray['totalitems'];
                            } else {
                                $aData[$akey]['week'] = (isset($aData[$akey]['week']) ? $aData[$akey]['week'] : $akey);
                                $aData[$akey]['totalitems'] = $itemArray['totalitems'];
                                $totalItems                 += $itemArray['totalitems'];
                            }
                        }
                        else {
        ...
        {
            //Total result
            $text = '<style="font-size:12px;">Weekly Report For</style> <br/><style="font-size:14px;">HQ</style><br/><style="font-size:12px;"> Week ' . $fromWeekNumber . ' - Week ' . $endFromWeekNumber . '</style>';
            $subTitle = '<style="font-size:14px;font-weight:bold;">Total ' . $totalItems . ' Items </style>';
            ?>
            <div class="col-lg-12">
                <div id="cTotal"></div>
            </div>
                    ...
                    $(document).ready(function() {
                        var colors      = Highcharts.getOptions().colors,
                            ...
                            data        = [
                                ...
                                ?>
                                {
                                    y: <?php echo (isset($graphdata['totalitems'])) ? $graphdata['totalitems']:0;?>,
                                    color: '#f79d54',
                                    drilldown: {                        
                                        ...
                                        foreach($graphdata['daywise'] as $key => $innerGraphdata){
                                            if($splinestring==''){
                                                $splinestring .= (isset($innerGraphdata['totalitems'])) ? $innerGraphdata['totalitems'] : 0;    
                                            }else{
                                                $splinestring .= ","; 
                                                $splinestring .= (isset($innerGraphdata['totalitems'])) ? $innerGraphdata['totalitems'] : 0;
                                        ...
                                        if(isset($graphdata['totalitems'])){
                                            if($ordermax < $graphdata['totalitems']){
                                                $ordermax = $graphdata['totalitems'];
                                            }
                                        }
                                        ?>
    ...
    foreach($pageMerchant[$graphPageNumber] as $keyStore => $valueStore)
    {
        ...
        if(isset($graphData['Store'][$keyStore]))
        {
            foreach($graphData['Store'][$keyStore] as $keyTot => $graphdatatTot){
                if (isset($graphdatatTot) && !empty($graphdatatTot)) {
                    if(array_key_exists('totalitems', $graphdatatTot) && isset($graphdatatTot['totalitems']))
                    {
                        $aData[$keyTot]['daywise'] = $graphdatatTot['daywise'];
                        if(isset($aData[$keyTot]['totalitems']))
                        {
                            $totalItems                += $graphdatatTot['totalitems'];
                        } else {
                            $totalItems                 += $graphdatatTot['totalitems'];
                        }
                    } else {
        ...
        //$text = '<style="font-size:12px;">' . $valueStore . ' weekly report for Week ' . $fromWeekNumber . ' - Week ' . $endFromWeekNumber . '</style>';
        $subTitle = '<style="font-size:14px;font-weight:bold;">Total ' . $totalItems . ' Items </style>';
        ?>
        <div class="col-lg-4">
