...
if (!empty($graphData['Store']) && !empty($stores)) {
    ...
    if($graphPageNumber == 0)
    {
            ...
            if (isset($graphDataAll))
            {
                ...
                if(!empty($result1))
                {
                    foreach($result1 as $coupon)
                    {
                        ...
                        if(array_key_exists(date('n',strtotime($coupon['order_date'])), $tcoupon))
                        {
        ...
        if (!empty($aData) && !empty($datee)) 
        {
            ...
            $totalItem = array_sum($aData);
            $text = '<style="font-size:12px;">Monthly Report for</style> <br/><style="font-size:14px;">HQ</style><br/><style="font-size:12px;">' . str_replace('\'', '', $datee[$month]) . ' - ' . $year . '</style>';
...
<div id="pagination_data_request">
    <?php echo $this->element('hqsalesreports/coupon/paginationall'); ?>
</div>